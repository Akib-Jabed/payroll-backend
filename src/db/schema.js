import { mysqlTable, mysqlSchema, index, primaryKey, int, varchar, date, tinyint, mysqlEnum, timestamp, text, foreignKey, datetime, double, decimal, bigint, char, float, time, longtext, unique, mediumtext } from "drizzle-orm/mysql-core"
...
export const accVoucher = mysqlTable("acc_voucher", {
	...
	idTask: int("id_task").references(() => accAutoVoucherSetup.idAutoVoucherSetup),
...
export const hrisEmployeeStationaryRequisitionItemsDetails = mysqlTable("hris_employee_stationary_requisition_items_details", {
	idHrisEmployeeStationaryRequisitionItemsDetails: int("id_hris_employee_stationary_requisition_items_details").autoincrement().notNull(),
	idProject: int("id_project").notNull().references(() => projects.idProjects),
	idUsers: int("id_users").notNull().references(() => users.idUsers),
	idHrisEmployeeStationaryRequisitionDetails: int("id_hris_employee_stationary_requisition_details").notNull().references(() => hrisEmployeeStationaryRequisitionDetails.idHrisEmployeeStationaryRequisitionDetails),
...
(table) => [
	primaryKey({ columns: [table.idHrisEmployeeStationaryRequisitionItemsDetails], name: "hris_employee_stationary_requisition_items_details_id_hris_employee_stationary_requisition_items_details"}),
]);
...
export const invItems = mysqlTable("inv_items", {
	idInvItems: int("id_inv_items").autoincrement().notNull(),
	itemCode: varchar("item_code", { length: 80 }).notNull(),
	itemCodeType: mysqlEnum("item_code_type", ['auto','manual']).default('auto'),
	itemName: varchar("item_name", { length: 300 }),
	itemDescription: text("item_description"),
	idProducts: int("id_products").references(() => products.idProducts),
	idCategories: int("id_categories").notNull().references(() => categories.idCategories),
	idSize: int("id_size").references(() => size.idSize),
	idColor: int("id_color").references(() => color.idColor),
	idSpecifications: int("id_specifications").references(() => specifications.idSpecifications),
	idUnit: int("id_unit").references(() => unit.idUnit),
	idModels: int("id_models").references(() => models.idModels),
	origin: varchar({ length: 100 }),
	weight: double(),
	partNumber: varchar("part_number", { length: 100 }),
	others: text(),
	remarks: text(),
	isSales: tinyint("is_sales").default(0).notNull(),
	isPurchase: mysqlEnum("is_purchase", ['yes','no']).default('no').notNull(),
	isScrap: mysqlEnum("is_scrap", ['yes','no']).default('no').notNull(),
	isFixedAsset: mysqlEnum("is_fixed_asset", ['yes','no']).default('no').notNull(),
	isNonStock: mysqlEnum("is_non_stock", ['yes','no']).default('no').notNull(),
	isManufactured: mysqlEnum("is_manufactured", ['yes','no']).default('no').notNull(),
	isRawMaterial: mysqlEnum("is_raw_material", ['yes','no']).default('no').notNull(),
	isShutterMaterial: mysqlEnum("is_shutter_material", ['yes','no']).default('no').notNull(),
	assetClassId: int("asset_class_id"),
	assetType: mysqlEnum("asset_type", ['land','land_development']),
	shutterType: varchar("shutter_type", { length: 100 }),
	idSecondaryUnit: int("id_secondary_unit").references(() => unit.idUnit),
	activeStatus: mysqlEnum("active_status", ['active','inactive']).default('active').notNull(),
	productSerialNo: varchar("product_serial_no", { length: 100 }),
	imei: varchar({ length: 100 }),
	enginCapacityCc: varchar("engin_capacity_cc", { length: 100 }),
	chesissNo: varchar("chesiss_no", { length: 150 }),
	regNo: varchar("reg_no", { length: 100 }),
	capacity: varchar({ length: 100 }),
	titleDeedNo: varchar("title_deed_no", { length: 100 }),
	titleDeedName: varchar("title_deed_name", { length: 150 }),
	// you can use { mode: 'date' }, if you want to have Date as type for this column
	titleDeedDate: date("title_deed_date", { mode: 'string' }),
	mouzaName: varchar("mouza_name", { length: 100 }),
	address: text(),
	idDistrict: int("id_district"),
	areaInDecimal: double("area_in_decimal"),
	titleDeedValue: decimal("title_deed_value", { precision: 17, scale: 2 }),
	scheduleOfLand: varchar("schedule_of_land", { length: 200 }),
	regCost: decimal("reg_cost", { precision: 17, scale: 2 }),
	depreciationCalStatus: mysqlEnum("depreciation_cal_status", ['yes','no']),
	depreciationMethod: varchar("depreciation_method", { length: 100 }),
	depreciationRate: double("depreciation_rate"),
	idDefaultImage: int("id_default_image").references(() => invItemImages.idInvItemImages),
	createDate: timestamp("create_date", { mode: 'string' }).defaultNow().notNull(),
	updateDate: timestamp("update_date", { mode: 'string' }).defaultNow().onUpdateNow().notNull(),
},
(table) => [
	index("id_categories").on(table.idCategories),
	index("id_default_image").on(table.idDefaultImage),
	index("id_models").on(table.idModels),
	index("id_secondary_unit").on(table.idSecondaryUnit),
	index("id_unit").on(table.idUnit),
	index("idx_id_color_items").on(table.idColor),
	index("idx_id_products_items").on(table.idProducts),
	index("idx_id_size_items").on(table.idSize),
	index("idx_id_speci_items").on(table.idSpecifications),
	index("idx_shutter_type").on(table.shutterType),
	index("item_code").on(table.itemCode),
	index("item_name").on(table.itemName),
	primaryKey({ columns: [table.idInvItems], name: "inv_items_id_inv_items"}),
]);
...
export const porItGoodsItemsDetails = mysqlTable("por_it_goods_items_details", {
	idPorItGoodsItemsDetails: int("id_por_it_goods_items_details").autoincrement().notNull(),
	idProject: int("id_project").notNull().references(() => projects.idProjects),
	idUsers: int("id_users").notNull().references(() => hrEmployee.employeeId),
	idPorItGoodsDetails: int("id_por_it_goods_details").notNull().references(() => porItGoodsDetails.idPorItGoodsDetails),
	idInvItems: int("id_inv_items").notNull().references(() => invItems.idInvItems),
	itemQuantity: int("item_quantity").notNull(),
	dateCreated: timestamp("date_created", { mode: 'string' }).defaultNow().notNull(),
	dateUpdated: timestamp("date_updated", { mode: 'string' }).defaultNow().onUpdateNow().notNull(),
...
(table) => [
	primaryKey({ columns: [table.idPorItGoodsItemsDetails], name: "por_it_goods_items_details_id_por_it_goods_items_details"}),
]);
